import { View, Text, ScrollView, TouchableOpacity, Image, TextInput, Dimensions, Linking } from 'react-native'
...
export const Project = ({ navigation, route }) => {
	...
	const getGole = async () => {
		try {
			const startOfWeek = moment().startOf('W').format('YYYY-MM-DD')
			const endOfWeek = moment().endOf('W').format('YYYY-MM-DD')
			const { data } = await axios.post(baseURL, {
				query: `
					query{
						projectGole(
				...
				<View style={styles.container}>
					...
					<View style={styles.body}>
						<View style={styles.bodyTop}>
							<View>
								<Text style={styles.topTxt}>Hi, {user_Infos.USER_NAME}</Text>
								<Text style={[styles.topTxt, { color: 'black' }]}>{getSayHello()}</Text>
							</View>
							<View style={styles.projectImgContainer}>
								<ProjectImage
									styles={styles.projectImg}
									source={projectInfo.PROFILE} />
							</View>
						</View>
										...
										<View style={styles.coworker} key={'index' + index}>
											...
											<View style={styles.coworkerIcons}>
												{member.TECHS.map((tech, index) => (
													<View key={index} style={styles.coworkerIcon}>{skillImg[tech](20)}</View>
												))}
											</View>
