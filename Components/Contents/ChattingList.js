        ...
        <ScrollView
          ...
          {groupChatList.length > 0 ?
            groupChatList.map((item, index) => {
              const profile = item.PROFILE ? { uri: item.PROFILE } : require('../../assets/tutorial/tutorial3.png')
              ...
              const date = moment.utc(chatInfoList?.groupChatList?.[item.ID]?.time ?? moment.utc().local().format()).local().format('YY.MM.DD')
